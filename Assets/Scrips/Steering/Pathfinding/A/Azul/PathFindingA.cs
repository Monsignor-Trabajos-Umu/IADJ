...
namespace Assets.Scrips.Steering.Pathfinding.A
{
    public class PathFindingA : MonoBehaviour
    {
        ...
        public void StartFindPath(Vector3 startPos, Vector3 targetPos,Heuristic heuristic,AgentNpc agente,FastGrid typeGrid)
        {
            
            StartCoroutine(FindPath(startPos, targetPos,heuristic,typeGrid,agente));
        }

        private IEnumerator FindPath(Vector3 startPos, Vector3 targetPos,Heuristic heuristic,FastGrid typeGrid,AgentNpc agente)
        {
            Debug.Log($"Calculating path from {startPos} {targetPos} using {heuristic.name}");
            var waypoints = new Vector3[0];
            var pathSuccess = false;

            var startNode = typeGrid.GetNodeFromWorldPoint(startPos);
            var targetNode = typeGrid.GetNodeFromWorldPoint(targetPos);


            if (!startNode.pared && !targetNode.pared)
            {
                var openSet = new Heap<NodeHeaped>(typeGrid.MaxSize);
                ...
                while (openSet.Count > 0)
                {
                    ...
                    foreach (var neighbor in typeGrid.GetNeighbors(currentNode))
                    {
                        if (neighbor.pared || closedSet.Contains(neighbor)) continue;

                        var newCostToNeighbor = currentNode.gCost + GetDistance(currentNode, neighbor,heuristic,agente);
                        ...
                        if (newCostToNeighbor < neighbor.gCost || !openSet.Contains(neighbor))
                        {
                            ...
                            neighbor.hCost = GetDistance(neighbor, targetNode,heuristic,agente);
            ...
            requestManagerA.FinishedProcessingPath(waypoints, pathSuccess);
        }
        ...
        // 0 nada
        // -1 enemigo
        private int MejorPeorTerreno(NodeHeaped nodeB,AgentNpc agente)
        {
        ...
        private float GetDistance(NodeHeaped nodeA, NodeHeaped nodeB,Heuristic heuristic,AgentNpc agente)
        {
            ...
            var extraTerreno = MejorPeorTerreno(nodeB,agente) switch
            {
